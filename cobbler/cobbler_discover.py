...
def dhcp_callback(pkt):
  ...
  #   yes - do nothing
  #   no  - add the node to cobbler using the basename and profile specified

  # RFC2132 DHCPDISCOVER == 1
  if pkt[DHCP] and pkt[DHCP].options[0][1] == 1:
    mymac = pkt[Ether].src

    # list all known MAC and IP addresses
    mac = []
    ip  = []
    ...
    if mymac in mac:
      ...
      # TODO: determine uniqueName and uniqueIP, netmask, dns-name, gateway
      #cobbler system add --name=uniqueName --netboot-enabled=true --hostname=uniqueName --gateway=fromdhcp --mac-address=mymac --ip-address=uniqueIP --netmask=fromdhcp --dns-name=fromdhcp --interface=eth0

  # RFC2132 DHCPOFFER == 2
  # Pull the DHCP server, router, and subnet mask from the offer
  # should be other ways to get this...like cobbler api queries? or parsing /etc/dhcpd/dhcpd.conf...
