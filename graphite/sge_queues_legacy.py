...
try:
  ...
  os.environ["SGE_CELL"]
  os.environ["SGE_ARCH"]
  os.environ["SGE_EXECD_PORT"]
  os.environ["SGE_QMASTER_PORT"]
except:
  os.environ["SGE_ROOT"] = "/opt/gridengine"
  os.environ["SGE_CELL"] = "default"
  os.environ["SGE_ARCH"] = "lx24-amd64"
...
def parse_qstat(name):
  ...
  [ queue, cqload, used, avail, total, aoacds, cdsue ] = str.split(p2.communicate()[0])

  if cqload == "-NA-":
    cqload = 0.00

  return (cqload, used, avail, total)
...
for q in queues:
  lines = []
  (cqload, used, avail, total) = parse_qstat(q)
  q = q.replace(".","_")
