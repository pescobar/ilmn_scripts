...
for i in `seq 1 20`; do
	...
	rocks set host interface options ${BASENAME}${i} bond1 options="miimon=100 mode=0"

	# Add 10GbE bonded interfaces
	#rocks add host bonded ${BASENAME}${i} channel=bond0 interfaces=eth0,eth1 ip=${TENGIG}.${i} network=${ISIZONE}
